...
export const imageController = {
    ...
    async moveImages(c) {
        try {
            ...
            for (const file of results) {
                ...
                // 更新数据库记录
                await c.env.MY_DB.prepare(
                    'UPDATE images SET folder_path = ?, full_path = ?, updated_at = CURRENT_TIMESTAMP WHERE filename = ? AND user_id = ?'
    ...
    async updateImageDescription(c) {
        try {
            ...
            await c.env.MY_DB.prepare(
                'UPDATE images SET description = ?, updated_at = CURRENT_TIMESTAMP WHERE filename = ? AND user_id = ?'
